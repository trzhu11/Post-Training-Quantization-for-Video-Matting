...
class AdaRoundFakeQuantize(QuantizeBase):
    ...
    def forward(self, X):
        # --- 修改: 只在维度不是 2 或 4 时打印警告 ---
        if X.ndim != 4:
             error_message = f"错误: AdaRoundFQ ({getattr(self, 'name', 'N/A')}) 期望 4D 权重输入，但收到了 {X.ndim}D 形状 {X.shape}!"
             print(error_message)
             raise ValueError(error_message) # 抛出 ValueError 终止程序        # --- 结束修改 ---
        if self.observer_enabled == 1:
            self.observer(X.detach())
            _scale, _zero_point = self.observer.calculate_qparams(self.observer.min_val, self.observer.max_val)
            _scale, _zero_point = _scale.to(self.scale.device), _zero_point.to(self.zero_point.device)
