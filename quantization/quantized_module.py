...
class QuantizedLayer(QuantizedModule):
    def __init__(self, module, activation, w_qconfig, a_qconfig, qoutput=True):
        super().__init__()
        self.bn_folded = getattr(module, 'bn_folded_', False)
        # 获取原始 module 的输出通道数/特征数
        self.out_channels = None
        if hasattr(module, 'out_channels'): # Conv2d
            self.out_channels = module.out_channels
        elif hasattr(module, 'out_features'): # Linear
    ...
    def forward(self, x):
        ...
        return x
    def new_forward(self, x, *args_inner, **kwargs_inner):
        x_orig = self.module(x); x_aff = x_orig
        if getattr(self, 'affine_enabled_runtime', False): x_aff = x_orig * self.gamma_prime_runtime + self.beta_prime_runtime
        x_act = x_aff
        if self.activation is not None:
            if not isinstance(self.activation, StraightThrough): x_act = self.activation(x_aff)
        x_final = x_act
        if self.qoutput and hasattr(self, 'layer_post_act_fake_quantize') and self.layer_post_act_fake_quantize is not None:
            x_final = self.layer_post_act_fake_quantize(x_act)
        elif hasattr(self, 'act_quantizer') and self.act_quantizer is not None:
            x_final = self.act_quantizer(x_act)
        return x_final
